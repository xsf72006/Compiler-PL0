...
本编译系统采用**C++**语言编写。编译系统的编译器部分由词法分析、语法分析、语义分析及代码生成、符号表管理、错误处理五部分组成。整个编译器以语法分析程序为中心，调用词法分析程序识别单词，生成相应的代码，查询或填写相应的符号表项，在出错时调用错误处理程序，报告错误，同时跳过出错部分继续进行语法分析。
